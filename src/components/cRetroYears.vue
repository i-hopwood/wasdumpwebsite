    ...
    <div class="accordion shadow-sm" id="pastYearsAccordion" v-else>
      ...
      >
        <h2 class="accordion-header" :id="'headingYear' + year.yearNumber">
          <button
            class="accordion-button collapsed bg-dark text-white border-bottom border-secondary"
            type="button"
            ...
            <div class="mb-4">
              ...
              <div class="row g-3">
                <div
                  ...
                  v-for="console in year.consolesPlayed"
                  :key="console.consoleName"
                >
                  <div
                    class="p-3 border border-secondary rounded bg-secondary bg-opacity-10 text-center h-100 d-flex flex-column justify-content-center"
                  >
                    <span class="d-block text-muted small text-uppercase mb-1">{{
                      console.consoleName
                    }}</span>
                    <strong class="text-light fs-5">{{
                      console.playerWins.length ? console.playerWins[0].player : 'N/A'
                    }}</strong>
                  </div>
                </div>
              </div>
              ...
              <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                  <tr v-for="standing in year.finalStandings" :key="standing.player">
                    ...
                    <td class="fw-bold" :class="standing.rank === 1 ? 'text-warning' : ''">
                      {{ standing.player }}
                    </td>
...
export default {
  ...
  setup() {
    const processStandingsData = (data) => {
      ...
      const getGameDetails = (game) => {
        ...
        const scores = (game.Scores || game.scores || [])
          .map((s) => {
            ...
            const gotBonus =
              s.GotBonus !== undefined ? s.GotBonus : s.gotBonus !== undefined ? s.gotBonus : false
      ...
      const consolesPlayed = Object.values(consolesMap)
        .map((c) => ({
