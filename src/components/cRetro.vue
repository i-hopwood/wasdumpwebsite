    ...
    <div v-if="localResults" class="animate-fade-in">
      ...
      <div class="current-year-section mb-5">
          ...
          <div class="col-lg-8">
            <div class="card h-100 shadow-sm border-primary border-opacity-25">
              <div class="card-header bg-dark">
                <h4 class="mb-0"><i class="bi bi-trophy me-2 text-warning"></i>Current Standings</h4>
              </div>
              <div class="card-body p-0">
                <div class="table-responsive">
                  <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                      <tr v-for="standing in (localResults.finalStandings || localResults.finalstandings)" :key="standing.player">
                        <td class="ps-4 fw-bold text-secondary">#{{ standing.rank || standing.Rank }}</td>
                        <td class="fw-bold">{{ standing.player || standing.Player }}</td>
                        <td class="text-center fw-bold text-primary">{{ standing.totalPoints ?? standing.totalpoints }}</td>
                        <td class="text-center">{{ standing.totalWins ?? standing.totalwins }}</td>
                        <td class="text-center pe-4">{{ standing.bonusCount ?? standing.bonuscount }}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
          ...
          <div class="col-lg-4" v-if="currentCompetition">
               <div class="card shadow-sm border-secondary h-100">
                 ...
                 <div class="card-body d-flex flex-column justify-content-center text-center">
                     ...
                     <h6 class="card-subtitle mb-4 text-muted text-uppercase letter-spacing-2">{{ currentCompetition.System }}</h6>
                     
                     <div class="p-3 bg-black bg-opacity-25 rounded mb-3 border border-secondary border-opacity-25">
                       <p class="card-text fst-italic text-light mb-0">"{{ currentCompetition.Goal }}"</p>
                     </div>
                     
                     <p class="card-text mt-auto"><small class="text-muted"><i class="bi bi-calendar3 me-1"></i>Date: {{ currentCompetition.CompetionDateString }}</small></p>
                 </div>
               </div>
          </div>
        </div>
    
        <div class="row mt-4" v-if="(localResults.consolesPlayed && localResults.consolesPlayed.length)">
          <div class="col-12">
            <div class="card border-secondary">
              <div class="card-header cursor-pointer d-flex justify-content-between align-items-center bg-dark" @click="showConsoles = !showConsoles" role="button">
                <h5 class="mb-0 text-white"><i class="bi bi-hdd-network me-2"></i>Console Standings</h5>
                <i class="bi" :class="showConsoles ? 'bi-chevron-up' : 'bi-chevron-down'"></i>
              </div>
              <div class="card-body bg-darker" v-if="showConsoles">
                <div class="row g-3">
                  <template v-for="(console, idx) in localResults.consolesPlayed" :key="console.consoleName">
                    <div :class="`col-md-${Math.max(6, 12 / numConsoleCols)} col-lg-${12 / numConsoleCols}`">
                      <div class="p-3 border border-secondary rounded h-100 bg-secondary bg-opacity-10">
                        <strong class="d-block text-secondary mb-2 border-bottom border-secondary pb-1">{{ console.consoleName }} <span class="badge bg-secondary text-dark float-end">{{ console.gamesPlayedCount }} games</span></strong>
                        <ul class="list-unstyled mb-0 small">
                          <li v-for="win in console.playerWins" :key="win.player" class="d-flex justify-content-between mb-1">
        ...
        <div class="row mt-4" v-if="(localResults.gamesPlayed && localResults.gamesPlayed.length)">
          <div class="col-12">
            <div class="card border-secondary">
              <div class="card-header cursor-pointer d-flex justify-content-between align-items-center bg-dark" @click="showGames = !showGames" role="button">
                <h5 class="mb-0 text-white"><i class="bi bi-collection-play me-2"></i>Games History</h5>
              ...
              <div class="card-body p-0" v-if="showGames">
                <div class="table-responsive" style="max-height: 500px; overflow-y: auto;">
                  <table class="table table-hover table-sm mb-0">
                    ...
                    <tbody>
                      <tr v-for="game in localResults.gamesPlayed" :key="game.gameName + game.competionDate">
                        <td class="ps-3 fw-medium text-light">{{ game.gameName }}</td>
                        <td><span class="badge bg-secondary text-dark bg-opacity-75">{{ game.console }}</span></td>
                        <td class="small text-muted">{{ game.competionDate }}</td>
                        <td class="text-primary">{{ game.winner }}</td>
                        <td class="pe-3">
                          <ul class="list-inline mb-0 small">
                            <li class="list-inline-item text-nowrap" v-for="score in game.scores" :key="score.player + score.points">
                              <span class="text-muted">{{ score.player }}:</span> <span class="text-white">{{ score.points }}</span>
                            </li>
                          </ul>
                        </td>
                      </tr>
      ...
      </div> <div class="card border-secondary shadow-sm">
        <div class="card-header cursor-pointer d-flex justify-content-between align-items-center bg-dark" @click="showPastYears = !showPastYears" role="button">
          <h3 class="mb-0 text-white"><i class="bi bi-archive me-2"></i>Previous Years</h3>
          <i class="bi fs-4 text-white" :class="showPastYears ? 'bi-chevron-up' : 'bi-chevron-down'"></i>
        </div>
...
export default {
  ...
  setup() {
    ...
    return { localResults, numConsoleCols, showConsoles, showGames, showPastYears, currentCompetition };
  }
};
</script>
